...
void main()
{
...
#ifdef SOB_TEAMTEX
	vec4 texTeam = texture(inTexTeam, outUV0);
	vec4 texStripe = texture(inTexStripe, outUV0);
...
#else
	vec4 texTeam = vec4(0.0);
	vec4 texStripe = vec4(0.0);
#endif
	...
	#ifdef SOB_TEAM	
		float isPaint = texTeam.a*texStripe.a;
	#else
		float isPaint = 1.0;
	#endif
#endif
	...
	vec4 paint = mix(inColTeam, paintBase, 1 - texTeam.a);
	paint = mix(inColStripe, paint, 1 - texStripe.a);
